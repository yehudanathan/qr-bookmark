...
export const getLinks = (options: QueryConstraint[] = []) => {
  ...
  if (user) {
    ...
    return get(query(dbRef, ...options)).then(
      (snapshot) => snapshot.val() as { [id: string]: Link }
    ) as Promise<{ [id: string]: Link }>;
  }
  return null;
};
...
export const getLinksOrderByTime = () => {
  ...
  if (user) {
    ...
    const dbRef = ref(database, userPath);
    // return dbRef;
    return get(query(dbRef, orderByChild("dateTime"))).then(
      (snapshot) => snapshot.val() as { [id: string]: Link }
    ) as Promise<{ [id: string]: Link }>;
  }
  return null;
...
export const getLinkID = async (link) => {
  ...
  if (user) {
    const userPath = `${basePath}/${user.uid}`;
    const dbRef = ref(database, userPath);
    const links = await get(query(dbRef)).then(
      (snapshot) => snapshot.val() as { [id: string]: Link }
    );
